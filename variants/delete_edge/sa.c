...
static bool edge_1g_opt(int (*edge)[2], const int nodes, const int lines, const int based_nodes, const int based_lines,
			const int height, const int width, const int groups, const int start_line, const int low_length,
			const long long ii)
{
  assert(groups != 1);
  if(! IS_EDGE(edge[start_line][0])) return false;
...
void exchange_edge(const int nodes, const int lines, int edge[lines][2], const int height,
		   const int width, const int groups, const int low_length, const long long ii)
{
  ...
  while(1){
    while(1){
      ...
      if(!IS_EDGE(edge[tmp_line[0]][0]) && !IS_EDGE(edge[tmp_line[1]][0])) continue;
        ...
        if(edge_1g_opt(edge, nodes, lines, based_nodes, based_lines, height, width, groups,
    ...
    printf("\nBefore:\n");
    for(int i=0;i<groups;i++){
      printf("a-b : %d,%d(%d) %d,%d(%d)\n",
	     WIDTH(edge[tmp_line[i*2]][0], height), HEIGHT(edge[tmp_line[i*2]][0], height), edge[tmp_line[i*2]][0],
	     WIDTH(edge[tmp_line[i*2]][1], height), HEIGHT(edge[tmp_line[i*2]][1], height), edge[tmp_line[i*2]][1]);
      printf("c-d : %d,%d(%d) %d,%d(%d)\n",
	     WIDTH(edge[tmp_line[i*2+1]][0], height), HEIGHT(edge[tmp_line[i*2+1]][0], height), edge[tmp_line[i*2+1]][0],
	     WIDTH(edge[tmp_line[i*2+1]][1], height), HEIGHT(edge[tmp_line[i*2+1]][1], height), edge[tmp_line[i*2+1]][1]);
	     }*/
    int tmp_edge[groups*2][2];
    int r  = getRandom(2);
    int rr = getRandom(2);
    bool is_edge[2] = {IS_EDGE(edge[tmp_line[0]][0]), IS_EDGE(edge[tmp_line[1]][0])};
    for(int i=0;i<groups;i++){
      for(int j=0;j<2;j++){
	...
	tmp_edge[i*2+1][j] = edge[tmp_line[i*2+1]][j];
      ...
      if(!is_edge[0]){
	// -a -b    -a  c             -a  d
	//  c  d -> -b  d (r=0) or ->  c -b (r=1)
	if(r == 0){
	  if(rr == 0){
	    // -a c    -a -c
	    // -b d ->  b  d
	    REVERSE(&tmp_edge[i*2  ][1]);
	    REVERSE(&tmp_edge[i*2+1][0]);
	  }
	  else{ // rr == 1
	    // -a c     a  c
	    // -b d -> -b -d
	    REVERSE(&tmp_edge[i*2  ][0]);
	    REVERSE(&tmp_edge[i*2+1][1]);
	  }
	}
	else{ // r == 1
	  if(rr == 0){
    ...
    bool flag = false;
    for(int i=0;i<2;i++)
      if(IS_EDGE(tmp_edge[i][0]) && DISTANCE(tmp_edge[i][0], tmp_edge[i][1], height) > low_length)
	flag = true;
	     ...
	     const int low_length, const int groups, const int *rotate_hash, const bool enable_fixed_temp)
{
  ...
  for(ii=0;ii<ncalcs;ii++){
    ...
    while(1){
      ...
      exchange_edge(nodes, lines, tmp_edge, height, width, groups, low_length, ii);
			     ...
			     const int low_length, const bool enable_bfs, const int *rotate_hash)
{
  ...
  for(int i=0;i<ESTIMATED_TIMES;i++){
    ...
    exchange_edge(nodes, lines, tmp_edge, height, width, groups, low_length, i);
