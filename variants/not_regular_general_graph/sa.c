...
long long sa(const int nodes, const int lines, const int degree, const int groups, double temp, 
	     ...
	     const bool hill_climbing_flag, const bool detect_temp_flag, double *max_diff_energy, int edge[lines][2],
	     int *diam, double *ASPL, const int cooling_cycle, const int added_centers, const int based_nodes,
	     long long *total_accepts, const bool is_simple_graph, const int num_degrees[nodes], const int algo)
{
  ...
  evaluation(nodes, based_nodes, groups, lines, degree, adj, diam, ASPL, added_centers, num_degrees, algo);
  ...
  for(ii=0;ii<ncalcs;ii++){
	...
	copy_edge((int *)tmp_edge_nsg, (int *)tmp_edge, lines*2);
      ...
      if(evaluation(nodes, based_nodes, groups, lines, degree, adj, &tmp_diam, &tmp_ASPL, added_centers,
		    num_degrees, algo))
	break;
      else{
	if(is_simple_graph)
...
double estimate_elapse_time(const int nodes, const int based_nodes, const int lines, const int degree,
			    ...
			    const bool is_simple_graph, const int num_degrees[nodes], const int algo)
{
  int diam;    // Not use
  double ASPL; // Not use
  int *adj = malloc(sizeof(int)*nodes*degree); // int adj[nodes][degree];
...
void check_current_edge(const int nodes, const int degree, const int lines, const int groups,
			...
			const int added_centers, const int num_degrees[nodes], const int algo)
{
  int diam;    // Not use
  double ASPL;
  ...
  if(! evaluation(nodes, based_nodes, groups, lines, degree, (int *)adj, &diam, &ASPL, added_centers,
		  num_degrees, algo))
