...
static void print_help(char *argv)
{
 ...
 [-F fixed_temperature] [-e deleted_edges] [-Y] [-M] [-h]\n", argv);
}
...
static void set_args(const int argc, char **argv, char *infname, int *low_length, char *outfname, 
		     ...
		     bool *enable_halfway, double *fixed_temp, int *width, int *height, int *max_degree, int *deleted_edges)
{
  if(argc < 3)
    print_help(argv[0]);

  int result;
  while((result = getopt(argc,argv,"f:W:H:D:R:o:s:n:w:c:g:C:BdF:YMhe:"))!=-1){
    switch(result){
    case 'f':
    ...
    case 'e':
      *deleted_edges = atoi(optarg);
      if(*deleted_edges != 0 && *deleted_edges != 1 && *deleted_edges != 2)
	ERROR("-e value == 0 or 1 or 2\n");
      break;
    case 'W':
      *width = atoi(optarg);
      if(*width <= 0)
...
static void count_degree(const int nodes, const int lines, const int edge[lines*2], int degree[nodes])
{
  for(int i=0;i<nodes;i++)
    degree[i] = 0;
...
static void delete_corner(const int deleted_edges, const int lines, const int based_lines, const int nodes,
			  const int width, const int height, const int groups, int edge[lines][2])
{
  if(deleted_edges == 1){ // When deleted_edges is 1, groups must be 1 or 2.
    if(groups == 1){
      for(int i=0;i<lines;i++){
	if(edge[i][0] == 0 || edge[i][1] == 0){ // left bottom
	  edge[lines][0] = (edge[i][0] == 0)? edge[i][1] : edge[i][0];
	  edge[i][0] = edge[i][1] = NO_EDGE;
	  break;
	}
      }
      for(int i=0;i<lines;i++){
        if(edge[i][0] == nodes-1 || edge[i][1] == nodes-1){ // right top
	  edge[lines][1] = (edge[i][0] == nodes-1)? edge[i][1] : edge[i][0];
	  edge[i][0] = edge[i][1] = NO_EDGE;
          break;
	}
      }
	...
	if(edge[i][0] == 0 || edge[i][1] == 0){ // left bottom
	  edge[lines][0] = (edge[i][0] == 0)? edge[i][1] : edge[i][0];
	  edge[i][0] = edge[i][1] = NO_EDGE;
	  int tmp_line = (i+based_lines < lines)? i+based_lines : i+based_lines-lines; // right top
	  edge[lines][1] = (edge[tmp_line][0] == nodes-1)? edge[tmp_line][1] : edge[tmp_line][0];
	  edge[tmp_line][0]  = edge[tmp_line][1] = NO_EDGE;
	  break;
	}
      }
    }
  }
  else if(deleted_edges == 2){
    if(groups == 1){
      for(int i=0;i<lines;i++){
	if(edge[i][0] == 0 || edge[i][1] == 0){ // left bottom
	  edge[lines][0] = (edge[i][0] == 0)? edge[i][1] : edge[i][0];
	...
	if(edge[i][0] == nodes-1 || edge[i][1] == nodes-1){ // right top
      ...
      for(int i=0;i<lines;i++){
	if(edge[i][0] == height-1 || edge[i][1] == height-1){ // left top
	  edge[lines+1][0] = (edge[i][0] == height-1)? edge[i][1] : edge[i][0];
	  edge[i][0] = edge[i][1] = NO_EDGE;
	  break;
	}
      }
      for(int i=0;i<lines;i++){
	if(edge[i][0] == nodes-height || edge[i][1] == nodes-height){ // right bottom
	  edge[lines+1][1] = (edge[i][0] == nodes-height)? edge[i][1] : edge[i][0];
	  edge[i][0] = edge[i][1] = NO_EDGE;
	  break;
	}
      }
	...
	if(edge[i][0] == height-1 || edge[i][1] == height-1){ // left top
	  edge[lines+1][0] = (edge[i][0] == height-1)? edge[i][1] : edge[i][0];
	  edge[i][0] = edge[i][1] = NO_EDGE;
	  int tmp_line = (i+based_lines < lines)? i+based_lines : i+based_lines-lines; // right bottom
	  edge[lines+1][1] = (edge[tmp_line][0] == nodes-height)? edge[tmp_line][1] : edge[tmp_line][0];
	  edge[tmp_line][0] = edge[tmp_line][1] = NO_EDGE;
	  break;
	}
      }
	...
	if(edge[i][0] == 0 || edge[i][1] == 0){ // left bottom
	  edge[lines][0] = (edge[i][0] == 0)? edge[i][1] : edge[i][0];
	  edge[i][0] = edge[i][1] = NO_EDGE;
	  int tmp_line = (i+based_lines*2 < lines)? i+based_lines*2 : i+based_lines*2-lines; // right top
	  edge[lines][1] = (edge[tmp_line][0] == nodes-1)? edge[tmp_line][1] : edge[tmp_line][0];
	  edge[tmp_line][0] = edge[tmp_line][1] = NO_EDGE;
	  tmp_line = (i+based_lines < lines)? i+based_lines : i+based_lines-lines; // left top
	  edge[lines+1][0] = (edge[tmp_line][0] == height-1)? edge[tmp_line][1] : edge[tmp_line][0];
	  edge[tmp_line][0] = edge[tmp_line][1] = NO_EDGE;
	  tmp_line = (i+based_lines*3 < lines)? i+based_lines*3 : i+based_lines*3-lines; // right bottom
	  edge[lines+1][1] = (edge[tmp_line][0] == nodes-height)? edge[tmp_line][1] : edge[tmp_line][0];
	  edge[tmp_line][0] = edge[tmp_line][1] = NO_EDGE;
	  break;
	}
  ...
  if(deleted_edges == 1){
    // printf("%d,%d %d,%d\n",
    //	   WIDTH(edge[lines][0],height), HEIGHT(edge[lines][0],height),
    //	   WIDTH(edge[lines][1],height), HEIGHT(edge[lines][1],height));
    //    printf("%d\n", DISTANCE(edge[lines][0], edge[lines][1], height));
    int length = DISTANCE(edge[lines][0], edge[lines][1], height);
    while(length > low_length){
      for(int i=0;i<lines;i++){
			...
			const int low_length, const int max_degree)
{
  ...
  int degree[nodes];
  count_degree(nodes, lines, edge, degree);
  for(int i=0;i<nodes;i++)
    if(degree[i] > max_degree)
      ERROR("Degree is over %d\n", degree[i]);
				  ...
				  const int low_length)
{
  ...
  if(min_num != 0){
    while(1){
      ...
      exchange_edge(nodes, lines, (int (*)[2])tmp_edge, height, width, groups, low_length, 0);
...
static void output_params(const int max_degree, const int groups, const int low_length, const int random_seed,
			  ...
			  const char *outfname, const double average_time, const int deleted_edges,
...
int main(int argc, char *argv[])
{
  ...
  int random_seed = 0, cooling_cycle = 1, groups = 1, deleted_edges = 0;
  ...
  set_args(argc, argv, infname, &low_length, outfname, &random_seed, &ncalcs, &max_temp,
	   ...
	   &enable_bfs, &enable_halfway, &fixed_temp, &width, &height, &max_degree, &deleted_edges);
  ...
  if(enable_infname){
    ...
    edge        = malloc(sizeof(int)*(lines+deleted_edges)*2);
  ...
  if(deleted_edges != 0)
    delete_corner(deleted_edges, lines, based_lines, nodes, width, height, groups, (int (*)[2])edge);
  ...
  output_params(max_degree, groups, low_length, random_seed, max_temp, min_temp, ncalcs,
		cooling_cycle, cooling_rate, infname, outfname, average_time, deleted_edges,
