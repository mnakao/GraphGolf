...
// Make it easier to choose a short distance
static int getImportantLine(const int based_lines, const int value[based_lines+1], const int groups)
{
  int r = getRandom(value[based_lines]);
  int low = 0, high = based_lines;
  while(1){
    int mid = (high + low)/2;
    if(value[mid] <= r && r < value[mid+1])
      return mid + based_lines * getRandom(groups);
    else if(value[mid] > r)
      high = mid - 1;
    else
      low = mid + 1;
  ...
  ERROR("ERROR!\n");
  return 0; // This line must be executed
}
		   ...
		   const int height, const int width, const int groups, const int low_length, const long long ii)
{
  ...
  int value[based_lines+1];
  value[0] = 0;
  for(int i=0;i<based_lines;i++)
    value[i+1] = (1+low_length) - DISTANCE(edge[i][0], edge[i][1], height) + value[i];
  
  while(1){
    while(1){
      tmp_line[0] = getImportantLine(based_lines, value, groups);
      tmp_line[1] = getImportantLine(based_lines, value, groups);
