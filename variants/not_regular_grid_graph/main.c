...
static void set_args(const int argc, char **argv, char *infname, int *low_length, char *outfname, 
		     ...
		     bool *enable_halfway, double *fixed_temp, int *width, int *height, int *max_degree, bool *enable_random)
{
  if(argc < 3)
    print_help(argv[0]);

  int result;
  while((result = getopt(argc,argv,"f:W:H:D:R:o:s:n:w:c:g:C:BdF:YMhr"))!=-1){
    switch(result){
    case 'r':
      *enable_random = true;
      break;
...
static void verify_graph(const int nodes, const int lines, const int edge[lines*2], const int height,
...
static void output_params(const int max_degree, const int groups, const int low_length, const int random_seed,
			  ...
			  const char *outfname, const double average_time, 
...
int main(int argc, char *argv[])
{
  ...
  set_args(argc, argv, infname, &low_length, outfname, &random_seed, &ncalcs, &max_temp,
	   ...
	   &enable_bfs, &enable_halfway, &fixed_temp, &width, &height, &max_degree, &enable_random);
  ...
  if(enable_infname){
    ...
    degree = malloc(nodes*sizeof(int));
    if(max_degree == NOT_N_DEFINED){
      for(int i=0;i<nodes;i++)
	degree[i] = 0;
    ...
    if(enable_random)
      for(int i=0;i<lines*INITIAL_TIMES;i++)
	exchange_edge(nodes, lines, (int (*)[2])edge, height, width, groups, low_length, 0);
  }
  else{
  ...
  verify_graph(nodes, lines, edge, height, low_length, max_degree);
  lower_bound_of_diam_aspl(&low_diam, &low_ASPL, width, height, max_degree, low_length);

  check_current_edge(nodes, lines, max_degree, edge, low_ASPL, low_diam, groups, height, based_height,
  		     enable_bfs, rotate_hash, degree);
  double average_time = estimated_elapse_time(nodes, lines, max_degree, edge, height, width, based_height, groups,
					      low_length, enable_bfs, rotate_hash, degree);
  if(enable_hill_climbing){
