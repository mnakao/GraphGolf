...
static void exchange_edge(const int nodes, const int lines, const int degree, int edge[lines][2],
			  ...
			  const double max_temp, const double min_temp, const double temp, const long long ii)
{
  int tmp_line[groups*2], based_lines = lines/groups;
...
static bool accept(const double new_ASPL, const double current_ASPL, const bool enable_fixed_temp, const double fixed_temp,
		   ...
		   const double max_temp, const double min_temp, const int groups, const long long ii)
{
	     ...
	     const int low_length, const int groups,  const int *rotate_hash)
{
  ...
  for(ii=0;ii<ncalcs;ii++){
    ...
    while(1){
      ...
      exchange_edge(nodes, lines, degree, tmp_edge, height, width, groups, low_length,
