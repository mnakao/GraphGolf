...
static void delete_edges(const int lines, const int deleted_edges, const int height, const int nodes,
			 const int groups, const int based_lines, int edge[lines*2])
{
  int tmp[deleted_edges], num = 0;
  while(deleted_edges != num){
    int r     = getRandom(based_lines);
    bool flag = false;
    for(int i=0;i<num;i++)
      if(tmp[i] == r)
	flag = true;
    ...
    for(int i=0;i<groups;i++){
      ...
      REVERSE(&edge[(r+based_lines*i)*2+1]);
    }
    tmp[num++] = r;
  }
  ...
  for(int i=0;i<nodes;i++)
    if(degree[i] == 0)
      ERROR("Number of deleted edges(%d) is too large\n", deleted_edges);

  //  for(int i=0;i<nodes;i++)
  //    printf("degree[%d] = %d : %d,%d\n", i, degree[i],
  //	   WIDTH(i, height), HEIGHT(i, height));
}
...
int main(int argc, char *argv[])
{
  ...
  else if(deleted_edges >= based_lines-max_degree)
    ERROR("Deleted edges(%d) is too big\n", deleted_edges);
