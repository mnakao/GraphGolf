...
static int get_degree(const int nodes, const int lines, int edge[lines][2])
{
  int node[nodes];
...
static void set_degrees(const int nodes, int num_degrees[nodes], const int lines, int edge[lines][2])
{
...
static void create_symmetric_edge_with_vertexes(int (*edge)[2], const int based_nodes, const int based_lines,
						...
						const int added_centers, int const nodes)
{
  int added_edges_to_center = degree/groups;
				  ...
				  const bool is_simple_graph, const int algo)
{
  ...
  set_degrees(nodes, num_degrees, lines, edge);
  int diam;    // Not use
  double ASPL; // Not use
  ...
  while(1){
    ...
    if(!is_simple_graph)
      create_adj(nodes, lines, degree, (const int (*)[2])edge, adj);
    if(evaluation(nodes, based_nodes, groups, lines, degree, (int *)adj, &diam, &ASPL, 0, num_degrees, algo))
      break;
    // NOT_RESTORE
  }
...

{
  ...
  for(int i=0;i<nodes;i++){
    if(degree != n[i]){
      PRINT_R0("This is not a regular graph.\n");
      break;
    }
  }
...
static void output_params(const int nodes, const int degree, const int groups, const int random_seed,
			  ...
			  const double average_time, const bool hill_climbing_flag, const int added_centers, const int algo)
{
#ifdef NDEBUG
...
int main(int argc, char *argv[])
{
  ...
  int degree = get_degree(based_nodes, based_lines, edge);
  ...
  if(groups == 1)
    check_current_edge(nodes, degree, lines, groups, based_nodes, edge, low_ASPL,
		       added_centers, num_degrees, algo);

  
  double average_time = estimate_elapse_time(nodes, based_nodes, lines, degree, groups, edge,
					     added_centers, is_simple_graph, num_degrees, algo);
  ...
  output_params(nodes, degree, groups, random_seed, max_temp, min_temp, 
		...
		average_time, hill_climbing_flag, added_centers, algo);
  // Optimization
  timer_clear_all();
  ...
  long long step = sa(nodes, lines, degree, groups, max_temp, ncalcs, cooling_rate, low_diam, low_ASPL,
		      ...
		      cooling_cycle, added_centers, based_nodes, &num_accepts, is_simple_graph, num_degrees, algo);
  timer_stop(TIMER_SA);
