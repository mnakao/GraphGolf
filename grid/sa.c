...
static void print_result_header()
{
  ...
  PRINT_R0("Cur. Dia. GAP\t\tBest Dia. GAP\tCur. Len. Gap\tAccept Rate\n");
}
...
static void print_results(const long long num, const double temp, const double current_ASPL, 
			  ...
			  const int best_diam, const int low_diam, const int current_length,
			  const int length, const long long accepts, const long long rejects)
{
  PRINT_R0("%8lld\t%f\t", num, temp);
  PRINT_R0("%f ( %f )\t%f ( %f )\t%d ( %d )\t\t\t%d ( %d )\t\t%d (%d)\t\t",
...
{
  double diff = (current_ASPL-ASPL)*nodes*(nodes-1) + (current_total_over_length - total_over_length);
  if(diff >= 0){
    *accepts += 1;
...
static void calc_length(const int lines, int edge[lines][2], const int height,
			const int length, int *current_length, int *total_over_length)
{
  *current_length = 0;
  *total_over_length = 0;
  ...
  for(int i=0;i<lines;i++){
    ...
    *current_length = MAX((*current_length), distance);
    if(distance > length)
      *total_over_length += (distance-length);
      //      *total_over_length += distance;
  }
}
...
long long sa(const int nodes, const int lines, double temp, const long long ncalcs,
	     ...
	     const int cooling_cycle, long long *total_accepts, const int height, const int length)
{
  int degree = 2 * lines / nodes;
  int current_length = 0, total_over_length = 0;
  ...
  calc_length(lines, edge, height, length, &current_length, &total_over_length);
  
  double current_ASPL = *ASPL;
  double best_ASPL    = *ASPL;
  int current_diam    = *diam;
  int best_diam       = *diam;
  int best_total_over_length    = total_over_length;
  ...
  for(i=0;i<ncalcs;i++){
    if(i % print_interval == 0 && !detect_temp_flag){
      print_results(i, temp, current_ASPL, best_ASPL, low_ASPL, current_diam, best_diam,
		    low_diam, current_length, length, accepts, rejects);
      accepts = 0;
      rejects = 0;
    }
    ...
    calc_length(lines, tmp_edge, height, length, &current_length, &total_over_length);
	...
	best_total_over_length = total_over_length;
      }

      if(best_ASPL == low_ASPL && best_total_over_length == 0){
	if(!detect_temp_flag){
	  print_results(i, temp, current_ASPL, best_ASPL, low_ASPL, current_diam, best_diam,
			low_diam, current_length, length, accepts, rejects);
